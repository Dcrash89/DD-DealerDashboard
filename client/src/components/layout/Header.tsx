...
const Header: React.FC<HeaderProps> = ({ theme, toggleTheme, lang, setLang, t, onMenuClick }) => {
  const { user } = useAuth();
  ...
  const getWelcomeMessage = () => {
    if (!user) return '';
    switch (user.role) {
      case 'ADMIN': return t('adminWelcome');
      case 'DEALER': return t('dealerWelcome');
      case 'GUEST': return t('guestWelcome');
      default: return '';
    }
  };
  ...
  const getSubtitle = () => {
    if (!user) return '';
    switch (user.role) {
      case 'ADMIN': return t('adminSubtitle');
      case 'DEALER': return t('dealerSubtitle');
      case 'GUEST': return t('guestSubtitle');
      default: return '';
    }
  };

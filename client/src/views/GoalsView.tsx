...
const GoalsView: React.FC<GoalsViewProps> = ({ t }) => {
  ...
  const handleUpdate = (updatedData: Partial<Omit<Goal, 'id'>>) => {
      ...
      updateGoal(editingGoal.id, updatedData);
      setEditingGoal(null);
  }
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {goals.length > 0 ? goals.map((goal) => (
                <tr key={goal.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{new Date(goal.startDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{new Date(goal.endDate).toLocaleDateString()}</td>

...
const AddressBookView: React.FC<AddressBookViewProps> = ({ t }) => {
  const { dealers, addContact, updateContact, deleteContact } = useData();
  ...
  const handleSaveContact = (contactData: Omit<Contact, 'id' | 'dealerId'> | Partial<Omit<Contact, 'id'>>) => {
    if (editingContact) {
      updateContact(editingContact.dealerId, editingContact.contact.id, contactData);
    } else if (addingContactToDealerId) {
      addContact(addingContactToDealerId, contactData as Omit<Contact, 'id' | 'dealerId'>);
    }
    handleCloseModal();
  };
  ...
  const handleDeleteContact = (dealerId: string, contactId: string) => {
    if (window.confirm(t('confirmDeleteContactMessage'))) {
      deleteContact(dealerId, contactId);
    }
  };

...
const AvailableFormsView: React.FC<AvailableFormsViewProps> = ({ t }) => {
  const { user } = useAuth();
  const { forms, addFormSubmission } = useData();
  const [isFillModalOpen, setIsFillModalOpen] = useState(false);
  const [selectedTemplateToFill, setSelectedTemplateToFill] = useState<Form | null>(null);
  
  const formTemplates = forms.filter(f => f.published && !f.archived);

  const handleFillForm = (template: Form) => {
    setSelectedTemplateToFill(template);
    setIsFillModalOpen(true);
  };

  const handleSaveSubmission = (data: Record<string, any>) => {
    if (!selectedTemplateToFill || !user || !user.dealerId) return;
    addFormSubmission({
      formId: selectedTemplateToFill.id,

...
const MyTeamView: React.FC<MyTeamViewProps> = ({ t }) => {
  const { currentDealer, addContact, updateContact, deleteContact } = useData();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingContact, setEditingContact] = useState<Contact | null>(null);

  if (!currentDealer) {
    return <PlaceholderView t={t} pageName={t('myTeam')} iconName="dealers" />
  }
  ...
  const handleSaveContact = (contactData: Omit<Contact, 'id'> | Partial<Omit<Contact, 'id'>>) => {
    if (editingContact) {
      updateContact(currentDealer.id, editingContact.id, contactData);
    } else {
      addContact(currentDealer.id, contactData as Omit<Contact, 'id' | 'dealerId'>);
    }
    handleCloseModal();
  };
  ...
  const handleDeleteContact = (contactId: string) => {
    if (window.confirm(t('confirmDeleteContactMessage'))) {
      deleteContact(currentDealer.id, contactId);
    }
  };

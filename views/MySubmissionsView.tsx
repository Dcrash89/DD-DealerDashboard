...
import { FormSubmission, FormTemplate, FormField, FormFieldType } from '../types';
...
const MySubmissionsView: React.FC<MySubmissionsViewProps> = ({ formSubmissions, formTemplates, t, onUpdateFormSubmission }) => {
  
  const templatesWithSubmissions = useMemo(() => {
    const templateIds = new Set(formSubmissions.map(s => s.templateId));
    return formTemplates.filter(t => templateIds.has(t.id) && !t.archived);
  }, [formSubmissions, formTemplates]);
  ...
  // State for the portal-based dropdown
  ...
  const activeTemplate = useMemo(() => {
    return formTemplates.find(t => t.id === activeTemplateId);
  }, [activeTemplateId, formTemplates]);

  const submissionsForActiveTemplate = useMemo(() => {
    if (!activeTemplateId) return [];
    return formSubmissions.filter(s => s.templateId === activeTemplateId);
  ...
  const handleSaveSubmissionUpdate = (data: Record<string, any>) => {
    ...
    onUpdateFormSubmission(editingSubmission.id, { data });
  ...
  const handleQuickEditChange = (submission: FormSubmission, fieldId: string, newValue: any) => {
    ...
    if (needsFullModal) {
        ...
        onUpdateFormSubmission(submission.id, { data: newData });
    }
  }
    ...
    <>
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {submissionsForActiveTemplate.map(submission => (
                                <tr key={submission.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{submission.submissionDate}</td>
                                    {activeTemplate?.fields.map(field => (
      ...
      {editingSubmission && (
        <FormFillModal
            ...
            template={formTemplates.find(t => t.id === editingSubmission.templateId) || null}

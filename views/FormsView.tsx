...
import { FormTemplate, FormField, FormSubmission, Dealer, FormStatus } from '../types';
...
interface FormsViewProps {
  ...
  dealers: Dealer[];
  onAddFormTemplate: (templateData: Omit<FormTemplate, 'id' | 'fields' | 'published' | 'dealerCanEditSubmissions' | 'archived'>) => void;
  onUpdateFormTemplate: (templateId: string, updatedData: Partial<Omit<FormTemplate, 'id' | 'fields'>>) => void;
  onUpdateTemplateFields: (templateId: string, fields: FormField[]) => void;
  onUpdateFormSubmission: (submissionId: string, updatedData: Partial<Omit<FormSubmission, 'id' | 'templateId' | 'dealerId' | 'dealerName' | 'submissionDate'>>) => void;
  onArchiveFormTemplate: (templateId: string) => void;
  onCloneFormTemplate: (templateId: string) => void;
  onDeleteTemplatePermanently: (templateId: string) => void;
}
...
const FormsView: React.FC<FormsViewProps> = ({
  t, formTemplates, formSubmissions, dealers, onAddFormTemplate, onUpdateFormTemplate, onUpdateTemplateFields, onUpdateFormSubmission, onArchiveFormTemplate, onCloneFormTemplate, onDeleteTemplatePermanently
}) => {
  ...
  const handleAddField = (field: FormField) => {
    ...
    onUpdateTemplateFields(editingTemplate.id, newFields);
  };
  ...
  const renderSubmissionsForTemplate = () => {
      ...
      const submissions = formSubmissions.filter(s => s.templateId === editingTemplate.id);
                ...
                <div key={sub.id} className="bg-white dark:bg-gray-700/50 p-3 rounded-lg">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">{t('submittedOn')} {sub.submissionDate}</p>
                        </div>
                        <div className="w-40">
                             <Dropdown 
                                options={getStatusOptions()}
                                selectedValue={sub.status}
                                onSelect={(newStatus) => onUpdateFormSubmission(sub.id, { status: newStatus as FormStatus })}
                             />
                        </div>
                    </div>
                </div>
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => onAddFormTemplate({ title: t('templateTitle'), description: '' })}
            ...
            <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-gray-50 dark:bg-gray-900 shadow-2xl z-50 p-6 flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4 my-6">
                     <div>
                        ...
                        <input type="text" value={editingTemplate.title} onChange={e => onUpdateFormTemplate(editingTemplate.id, {title: e.target.value})} className="w-full mt-1 p-2 border rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 focus:ring-dji-blue focus:border-dji-blue"/>
                     </div>
                     <div>
                        <label className="text-sm font-medium">{t('templateDescription')}</label>
                        <textarea value={editingTemplate.description} onChange={e => onUpdateFormTemplate(editingTemplate.id, {description: e.target.value})} className="w-full mt-1 p-2 border rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 focus:ring-dji-blue focus:border-dji-blue" rows={3}></textarea>
                     </div>
                      <div className="flex items-center space-x-8">
                        <div className="flex items-center">
                            <input
                                ...
                                onChange={(e) => onUpdateFormTemplate(editingTemplate.id, { published: e.target.checked })}
                        ...
                        <div className="flex items-center">
                            <input
                                ...
                                onChange={(e) => onUpdateFormTemplate(editingTemplate.id, { dealerCanEditSubmissions: e.target.checked })}
                                className="h-4 w-4 rounded border-gray-300 text-dji-blue focus:ring-dji-blue"
                            />

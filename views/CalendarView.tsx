...
import { UserRole, FormSubmission, FormTemplate, Dealer, FormStatus } from '../types';
...
const CalendarView: React.FC<CalendarViewProps> = ({ t, lang, userRole, formSubmissions, formTemplates, dealers, currentDealer }) => {
  const [selectedDealer, setSelectedDealer] = useState<string>('all');
  const [selectedActivityType, setSelectedActivityType] = useState<string>('all');
  const [selectedEvent, setSelectedEvent] = useState<FormSubmission | null>(null);
  ...
  const events = useMemo(() => {
    ...
    if (userRole === UserRole.DEALER && currentDealer) {
    ...
    return filteredSubmissions
      .filter(s => s.eventDate) // Only show submissions that have a designated event date
      .map(submission => {
        ...
        return {
          ...
          resource: submission, // The original submission object
        };
    });
  }, [formSubmissions, userRole, currentDealer, selectedDealer, selectedActivityType]);
  ...
  return (
    <div className="h-[calc(100vh-160px)] flex flex-col gap-6">
      ...
      <EventDetailsModal 
        ...
        template={formTemplates.find(t => t.id === selectedEvent?.templateId)}
        t={t}
      />
    </div>
  );
